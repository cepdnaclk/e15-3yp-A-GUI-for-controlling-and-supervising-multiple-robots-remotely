...
Robot_SharpIR::Robot_SharpIR( sensorCode _sensorType , uint8_t _sensorPin ) {
...
void Robot_SharpIR::setFilterRate(float rate) {
  if (rate > 0 && rate < 1) alpha = rate;
}

float Robot_SharpIR::getDistanceFloat(bool avoidBurstRead ) {
...
uint8_t Robot_SharpIR::getDistance(bool avoidBurstRead ) {
  return (uint8_t)getDistanceFloat(avoidBurstRead );
}


uint8_t Robot_SharpIR::getRawDistance( bool avoidBurstRead ) {
  uint8_t distance ;
  ...
  switch ( sensorType )
  {
    ...
    case GP2Y0A21YK0F :
      ...
      distance = (28400) / (1 + analogRead(pin));  // To avoid divide by zero error
