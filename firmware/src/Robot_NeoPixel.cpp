/*!
 * @file Robot_NeoPixel.cpp
 *
 * @mainpage Arduino Library for driving Robot NeoPixel addressable LEDs,
 * FLORA RGB Smart Pixels and compatible devicess -- WS2811, WS2812, WS2812B,
 * SK6812, etc.
 *
 * @section intro_sec Introduction
 *
 * This is the documentation for Robot's NeoPixel library for the
 ...
 * to control Robot NeoPixels, FLORA RGB Smart Pixels and compatible
 * devices -- WS2811, WS2812, WS2812B, SK6812, etc.
 *
 * Robot invests time and resources providing this open source code,
 * please support Robot and open-source hardware by purchasing products
 * from Robot!
 *
 * @section author Author
 *
 * Written by Phil "Paint Your Dragon" Burgess for Robot Industries,
 * with contributions by PJRC, Michael Miller and other members of the
 * open source community.
 *
 * @section license License
 *
 * This file is part of the Robot_NeoPixel library.
 *
 * Robot_NeoPixel is free software: you can redistribute it and/or
 * modify it under the terms of the GNU Lesser General Public License as
 * published by the Free Software Foundation, either version 3 of the
 * License, or (at your option) any later version.
 *
 * Robot_NeoPixel is distributed in the hope that it will be useful,
...
/*!
  ...
  @param   t  Pixel type -- add together NEO_* constants defined in
              Robot_NeoPixel.h, for example NEO_GRB+NEO_KHZ800 for
              NeoPixels expecting an 800 KHz (vs 400 KHz) data stream
              with color bytes expressed in green, red, blue order per
              pixel.
  @return  Robot_NeoPixel object. Call the begin() function before use.
*/
Robot_NeoPixel::Robot_NeoPixel(uint16_t n, uint8_t p, neoPixelType t) :
...
/*!
  @brief   Deallocate Robot_NeoPixel object, set data pin back to INPUT.
*/
Robot_NeoPixel::~Robot_NeoPixel() {
  free(pixels);
...
*/
void Robot_NeoPixel::begin(void) {
  if(pin >= 0) {
    pinMode(pin, OUTPUT);
...
/*!
  @brief   Change the length of a previously-declared Robot_NeoPixel
  ...
  @note    This function is deprecated, here only for old projects that
           ...
           type).
*/
void Robot_NeoPixel::updateLength(uint16_t n) {
...
/*!
  @brief   Change the pixel format of a previously-declared
           Robot_NeoPixel strip object. If format changes from one of
  ...
  @note    This function is deprecated, here only for old projects that
           ...
           (length, pin, type).
*/
void Robot_NeoPixel::updateType(neoPixelType t) {
...
// ---------- END of Constants for cycle counter implementation --------
  ...
  if ( pwm != NULL ) {
    #ifdef ARDUINO_NRF52_Robot // use thread-safe malloc
...
//    pwm->INTEN |= (PWM_INTEN_SEQEND0_Enabled<<PWM_INTEN_SEQEND0_Pos);
    ...
    #ifdef ARDUINO_NRF52_Robot  // use thread-safe free
...
*/
void Robot_NeoPixel::setPin(uint8_t p) {
...
*/
void Robot_NeoPixel::setPixelColor(
...
*/
void Robot_NeoPixel::setPixelColor(uint16_t n, uint32_t c) {
  if(n < numLEDs) {
    uint8_t *p,
      r = (uint8_t)(c >> 16),
      g = (uint8_t)(c >>  8),
...
*/
void Robot_NeoPixel::fill(uint32_t c, uint16_t first, uint16_t count) {
  uint16_t i, end;
...
*/
uint32_t Robot_NeoPixel::ColorHSV(uint16_t hue, uint8_t sat, uint8_t val) {
...
*/
uint32_t Robot_NeoPixel::getPixelColor(uint16_t n) const {
...
*/
void Robot_NeoPixel::setBrightness(uint8_t b) {
...
*/
uint8_t Robot_NeoPixel::getBrightness(void) const {
  return brightness - 1;
}
...
*/
void Robot_NeoPixel::clear(void) {
  memset(pixels, 0, numBytes);
}

// A 32-bit variant of gamma8() that applies the same function
// to all components of a packed RGB or WRGB value.
uint32_t Robot_NeoPixel::gamma32(uint32_t x) {
  uint8_t *y = (uint8_t *)&x;
