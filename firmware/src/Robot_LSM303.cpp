...
Robot_LSM303::Robot_LSM303(void)
{
  /*
  ...
  */
  m_min = (Robot_LSM303::vector<int16_t>){-32767, -32767, -32767};
  m_max = (Robot_LSM303::vector<int16_t>){+32767, +32767, +32767};
...
bool Robot_LSM303::timeoutOccurred()
{
...
void Robot_LSM303::setTimeout(unsigned int timeout)
{
  io_timeout = timeout;
}

unsigned int Robot_LSM303::getTimeout()
{
  return io_timeout;
}

bool Robot_LSM303::init(deviceType device, sa0State sa0)
{
  // perform auto-detection unless device type and SA0 state were both specified
  if (device == device_auto || sa0 == sa0_auto)
  {
    // check for Robot_LSM303D if device is unidentified or was specified to be this type
    ...
    // check for Robot_LSM303DLHC, DLM, DLH if device is still unidentified or was specified to be one of these types
...
*/
void Robot_LSM303::enableDefault(void)
{
...
void Robot_LSM303::writeAccReg(byte reg, byte value)
{
...
byte Robot_LSM303::readAccReg(byte reg)
{
  byte value;
...
void Robot_LSM303::writeMagReg(byte reg, byte value)
{
...
byte Robot_LSM303::readMagReg(int reg)
{
  byte value;
...
void Robot_LSM303::writeReg(byte reg, byte value)
{
...
byte Robot_LSM303::readReg(int reg)
{
  // mag address == acc_address for LSM303D, so it doesn't really matter which one we use.
...
void Robot_LSM303::readAcc(void)
{
  Wire.beginTransmission(acc_address);
...
void Robot_LSM303::readMag(void)
{
  Wire.beginTransmission(mag_address);
...
// Reads all 6 channels of the Robot_LSM303 and stores them in the object variables
void Robot_LSM303::read(void)
{
  readAcc();
  readMag();
}
...
*/
float Robot_LSM303::heading(void)
{
  if (_device == device_D)
  {
...
void Robot_LSM303::vector_normalize(vector<float> *a)
{
...
int Robot_LSM303::testReg(byte address, regAddr reg)
{
